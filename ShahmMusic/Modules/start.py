...
from coffee import Telegram, YouTube, app
from coffee.misc import SUDOERS, _boot_
from coffee.plugins.playlist import del_plist_msg
from coffee.plugins.sudoers import sudoers_list
from coffee.utils.database import (add_served_chat,
                                       add_served_user,
                                       get_served_chats,
                                       get_served_users,
                                       blacklisted_chats,
...
from coffee.utils.decorators.language import LanguageStart
from coffee.utils.formatters import get_readable_time
from coffee.utils.inline import (help_pannel, private_panel,
                                     start_pannel)
        ...
        if name[0:3] == "sta":
            ...
            def get_stats():
                ...
                for vidid, count in list_arranged.items():
                    ...
                    if vidid == "telegram":
                        msg += f"🔗[قناة السورس](https://t.me/K_OK0KK) ** ᴩʟᴀʏᴇᴅ {count} ᴛɪᴍᴇs**\n\n"
                    else:
...
💖 بحث بواسطة {config.MUSIC_BOT_NAME}"""
            key = InlineKeyboardMarkup(
                [
                    [
                        ...
                        InlineKeyboardButton(
                            text="• قناة السورس •", url="https://t.me/K_OK0KK"
                        ),
                    ],
                ]
            )
        ...
        if config.START_IMG_URL:
            try:
                ...
                user = message.from_user.id
                if int(user) == dev_owner: 
                   return await message.reply(f"**𖢿 | : مرحبا حبيبي كوفي مطور السورس{message.from_user.mention}\n𖢿 | : كل اقسام التحكم بالبوتات\n𖢿 | : تستطيع التحكم بكل البوتات عن طريق هذه الازرار**",reply_markup=OwnerM)
